...
system qw(docker exec --user kasm-user -w), $experts, qw(mt5 rm -fr MoneyMonster);
system qw(docker exec --user kasm-user -w), $experts, qw(mt5 mkdir MoneyMonster);

system qw(docker exec --user kasm-user -w), $mm, qw(mt5 bash -c), "tar jxf /tmp/files.tbz2 . 2>/dev/null";

system qw(docker exec --user kasm-user -w), $mm, qw(mt5 wine compile_mt5.bat);
